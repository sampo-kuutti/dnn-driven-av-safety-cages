...
NUM_INPUTS = 3  # number of input features, used for re-shaping input matrices
DATA_DIR = 'S:/Research/safeav/Supervised Learning/data/'  # directory data sets are stored in
...
class DataReader(object):
    ...
    """
    def __init__(self, data_dir=DATA_DIR, shuffle=SHUFFLE, num_in=NUM_INPUTS):
    ...
    def load(self):
        ...
        with open(self.data_dir + 'training_data.csv') as f:
        ...
        xs = array(xs).reshape(int(len(xs)/self.num_in), self.num_in)  # re-shape to a n x num_in matrix
    ...
    def load_train_batch(self, batch_size):
        ...
        x_out = array(x_out).reshape(batch_size, self.num_in)  # re-shape to a batch_size x 4 matrix
        return x_out, y_out
